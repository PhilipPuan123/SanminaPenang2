...
namespace ModbusTCP
{
    ...
    public class Master
    {
        ...
        /// <value>True if channel was disabled during connection.</value>
        ...
        public void Dispose()
        {
            ...
            _connected = false;         //Added: Set to false when disconnect
        }
        ...
        internal void CallException(ushort id, byte unit, byte function, byte exception)
        {
            ...
            if (exception == (byte)MError.excExceptionConnectionLost)
            {
                tcpSynCl = null;
                tcpAsyCl = null;
                _connected = false;         //Added: Set to false when connection lost
            }
        ...
        public void ReadCoils(ushort id, byte unit, ushort startAddress, ushort numInputs)
        {
            if (numInputs > 2000)
            {
                CallException(id, unit, (byte)ModbusFunctionCode.fctReadCoil, (byte)MError.excIllegalDataVal);
                return;
            }
            WriteAsyncData(CreateReadHeader(id, unit, startAddress, numInputs, (byte)ModbusFunctionCode.fctReadCoil), id);
        }
        ...
        public void ReadCoils(ushort id, byte unit, ushort startAddress, ushort numInputs, ref byte[] values)
        {
            ...
            values = WriteSyncData(CreateReadHeader(id, unit, startAddress, numInputs, (byte)ModbusFunctionCode.fctReadCoil), id);
        }
        ...
        public void ReadDiscreteInputs(ushort id, byte unit, ushort startAddress, ushort numInputs)
        {
            if (numInputs > 2000)
            {
                CallException(id, unit, (byte)ModbusFunctionCode.fctReadDiscreteInputs, (byte)MError.excIllegalDataVal);
                return;
            }
            WriteAsyncData(CreateReadHeader(id, unit, startAddress, numInputs, (byte)ModbusFunctionCode.fctReadDiscreteInputs), id);
        }
        ...
        public void ReadDiscreteInputs(ushort id, byte unit, ushort startAddress, ushort numInputs, ref byte[] values)
        {
            ...
            values = WriteSyncData(CreateReadHeader(id, unit, startAddress, numInputs, (byte)ModbusFunctionCode.fctReadDiscreteInputs), id);
        }
        ...
        public void ReadHoldingRegister(ushort id, byte unit, ushort startAddress, ushort numInputs)
        {
            if (numInputs > 125)
            {
                CallException(id, unit, (byte)ModbusFunctionCode.fctReadHoldingRegister, (byte)MError.excIllegalDataVal);
                return;
            }
            WriteAsyncData(CreateReadHeader(id, unit, startAddress, numInputs, (byte)ModbusFunctionCode.fctReadHoldingRegister), id);
        }
        ...
        public void ReadHoldingRegister(ushort id, byte unit, ushort startAddress, ushort numInputs, ref byte[] values)
        {
            ...
            values = WriteSyncData(CreateReadHeader(id, unit, startAddress, numInputs, (byte)ModbusFunctionCode.fctReadHoldingRegister), id);
        }
        ...
        public void ReadInputRegister(ushort id, byte unit, ushort startAddress, ushort numInputs)
        {
            if (numInputs > 125)
            {
                CallException(id, unit, (byte)ModbusFunctionCode.fctReadInputRegister, (byte)MError.excIllegalDataVal);
                return;
            }
            WriteAsyncData(CreateReadHeader(id, unit, startAddress, numInputs, (byte)ModbusFunctionCode.fctReadInputRegister), id);
        }
        ...
        public void ReadInputRegister(ushort id, byte unit, ushort startAddress, ushort numInputs, ref byte[] values)
        {
            ...
            values = WriteSyncData(CreateReadHeader(id, unit, startAddress, numInputs, (byte)ModbusFunctionCode.fctReadInputRegister), id);
        }
        ...
        public void WriteMultipleCoils(ushort id, byte unit, ushort startAddress, ushort numBits, byte[] values)
        {
            ...
            {
                CallException(id, unit, (byte)ModbusFunctionCode.fctWriteMultipleCoils, (byte)MError.excIllegalDataVal);
                return;
            }
        ...
        public void WriteMultipleRegister(ushort id, byte unit, ushort startAddress, byte[] values)
        {
            ...
            if (numBytes > 250)
            {
                CallException(id, unit, (byte)ModbusFunctionCode.fctWriteMultipleRegister, (byte)MError.excIllegalDataVal);
                return;
            }
        ...
        public void ReadWriteMultipleRegister(ushort id, byte unit, ushort startReadAddress, ushort numInputs, ushort startWriteAddress, byte[] values)
        {
            ...
            if (numBytes > 250)
            {
                CallException(id, unit, (byte)ModbusFunctionCode.fctReadWriteMultipleRegister, (byte)MError.excIllegalDataVal);
                return;
            }
        ...
        private void WriteAsyncData(byte[] write_data, ushort id)
        {
            if ((tcpAsyCl != null) && (tcpAsyCl.Connected))
            {
                ...
                {
                    CallException(id, write_data[6], write_data[7], (byte)MError.excExceptionConnectionLost);
                }
            }
            else CallException(id, write_data[6], write_data[7], (byte)MError.excExceptionConnectionLost);
        }
        ...
        private void OnSend(System.IAsyncResult result)
        {
            ...
            if (result.IsCompleted == false) CallException(0xFFFF, 0xFF, 0xFF, (byte)MError.excSendFailt);
        ...
        private void OnReceive(System.IAsyncResult result)
        {
            ...
            if (result.IsCompleted == false) CallException(0xFF, 0xFF, 0xFF, (byte)MError.excExceptionConnectionLost);
            ...
            // ------------------------------------------------------------
            // Write response data
            if ((function >= (byte)ModbusFunctionCode.fctWriteSingleCoil) && (function != (byte)ModbusFunctionCode.fctReadWriteMultipleRegister))
            {
                data = new byte[2];
            ...
            if (function > (byte)MError.excExceptionOffset)
            {
                function -= (byte)MError.excExceptionOffset;
        ...
        private byte[] WriteSyncData(byte[] write_data, ushort id)
        {
            ...
            if (tcpSynCl.Connected)
            {
                try
                {
                    ...
                    if (result == 0) CallException(id, unit, write_data[7], (byte)MError.excExceptionConnectionLost);
